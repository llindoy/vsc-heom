...
public:
    polaritonic_system(){}
    polaritonic_system(const rapidjson::Value& obj){CALL_AND_RETHROW(load(obj));}
    ...
    void qc_operator(linalg::matrix<T>& op, T Rdisp=0)
    {
        ...
        size_t nM = m_molsystem.Nm();
        linalg::matrix<T> mat(nM*_nF, nM*_nF);  mat.fill_zeros();
        for(size_t i = 0; i<nM; ++i)
        {
            for(size_t j = 0; j+1 < _nF; ++j)
            {
                size_t k = i*_nF+j;
                mat(k, k+1) = sqrt(j+1)/sqrt(2.0);                               
                mat(k+1, k) = sqrt(j+1)/sqrt(2.0);                               
            }
        }
        transform_operator(mat, op);
    }

    void qc2_operator(linalg::matrix<T>& op, T Rdisp=0)
    {
        op.resize(_nP, _nP);
        op.fill_zeros();
        ...
        linalg::matrix<T> mat2 = mat*mat;
        transform_operator(mat2, op);
    }
    ...
    void form_hamiltonian()
    {
        ...
        for(size_t i = 0; i < nM; ++i)
        {
            ...
            for(size_t j = 0; j < _nF; ++j)
            {
                //add on the diagonal terms
                size_t k = i*_nF+j;
                m_Hp(k, k) += _wc*(j+0.5);        //the matter terms - including the dipole self energy terms
            }
            ...
            for(size_t j = 0; j < _nF; ++j)
            {
                ...
                if(j + 1 != _nF)
                {
                    m_Hp(k, k+1) += xm(i)*chi*sqrt(j+1);
                    m_Hp(k+1, k) += xm(i)*chi*sqrt(j+1);
                }
            }
        }
    }
...
public:
    T cavity_frequency() const{return _wc;}
public:
