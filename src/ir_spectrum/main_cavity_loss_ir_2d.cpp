...
#include "../rkf.hpp"
#include "../polaritonic_system_2d.hpp"
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        polaritonic_system_2d<real_type> system(doc["system"]);
        ...
        real_type gammac = rapidjson_loader<real_type>::load(doc["gammac"])*from_cmn1;
        real_type Lambdac = 0; 
        {
            real_type cavity_lifetime = rapidjson_loader<real_type>::load(doc["cavitylifetime"])*from_fs;
            real_type loss_rate = 1.0/cavity_lifetime;
            real_type w0 = system.cavity_frequency();
            Lambdac = loss_rate/4.0 * (1-exp(-beta*w0)) * (gammac*gammac + w0*w0)/(w0*gammac);
        }
        ...
        size_type Kc = rapidjson_loader<size_type>::load(doc["kc"]);
        ...
        linalg::matrix<real_type> Q;
        system.Q_operator(Q);
        linalg::matrix<complex_type> ScoupQ(Q);

        linalg::matrix<real_type> qc;
        system.qc_operator(qc);
        linalg::matrix<complex_type> Scoupc(qc);


        linalg::matrix<real_type> dipo;
        system.dipole_operator(dipo);
        linalg::matrix<complex_type> dipole(dipo);
        ...
        //and the cavity bath renormalisation term
        ham += Lambdam*Q2;
        ...
        system.rho0(rrhobeta2, beta/2.0, Lambda, Lambdam, Lambdac);
        ...
        linalg::matrix<complex_type> rho0 = dipole*rhobeta2;


        std::vector<heom_bath<real_type>> terms(3);
        if(use_pade)
        {
            debye_bath_pade(Lambda, wc, beta, K, terms[0]);
            debye_bath_pade(Lambdac, gammac, beta, Kc, terms[1]);
            debye_bath_pade(Lambdam, gammam, beta, Km, terms[2]);
        }
        else
        {
        ...
        else
        {
            if(doc.HasMember("maxfactor"))
            {
                ...
                else
                {
                    ...
                    {
                        ASSERT(doc["maxfactor"].Size() == 3, "Invalid max factor array");
                        for(size_t i = 0; i < 3; ++i){ASSERT(doc["maxfactor"][i].IsNumber(), "Invalid max factor array.");}
                        std::vector<real_type> maxfactor(3);
                        maxfactor[0] = rapidjson_loader<real_type>::load(doc["maxfactor"][0]);
                        maxfactor[1] = rapidjson_loader<real_type>::load(doc["maxfactor"][1]);
                        maxfactor[2] = rapidjson_loader<real_type>::load(doc["maxfactor"][2]);

                        std::vector<real_type> scalefactor(3);  scalefactor[0] = 1; scalefactor[1] = 1; scalefactor[2] = 1;
                        try
                        {
                            if(doc.HasMember("scalefactor"))
                            {
                                ASSERT(doc["scalefactor"].IsArray(), "Invalid scalefactor.");
                                ASSERT(doc["scalefactor"].Size() == 3, "Invalid scalefactor.");
                                for(size_t i = 0; i < 3; ++i){ASSERT(doc["scalefactor"][i].IsNumber(), "Invalid scale factor array.");}
                                ...
                                scalefactor[2] = rapidjson_loader<real_type>::load(doc["scalefactor"][2]);
                            }
                        }
        ...
        auto Rv = dipole.reinterpret_shape(nhilb*nhilb);
        ...
        //thermalise
        {
            ...
            linalg::matrix<complex_type> k(4, ados.size());

            complex_type val0 = linalg::dot_product(linalg::conj(Rv), rhov);
            std::cout << "Running." << std::endl;
            std::cout << "0.0 " << linalg::real(val0/std::abs(val0)) << " " << linalg::imag(val0/std::abs(val0)) << std::endl;
            ...
            for(size_t i = 0; i < nsteps; ++i)
            {
                ...
                std::cout << (i+1)*dt/from_fs << " " << linalg::real(val/std::abs(val0)) << " " << linalg::imag(val/std::abs(val0)) << std::endl;
                //for(size_t j = 0; j < nhilb; ++j){ std::cout << std::real(ados[0](j, j)) << " ";}    std::cout << std::endl;
            }
        }
