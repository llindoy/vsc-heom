...
#include "../rkf.hpp"
#include "../polaritonic_system.hpp"
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        polaritonic_system<real_type> system(doc["system"]);
        ...
        size_type nequil = rapidjson_loader<size_type>::load(doc["nequil"]);
        ...
        real_type Lambdac = loss_rate/4.0 * (1-exp(-beta*w0)) * (gammac*gammac + w0*w0)/(w0*gammac);
        ...
        system.rho0(rrhobeta2, beta, Lambda);
        ...
        linalg::matrix<complex_type> rho0 = rhobeta2;
        ...
        auto Rv = Scoup.reinterpret_shape(nhilb*nhilb);
        ...
        linalg::matrix<complex_type> temp;
        for(size_t i =0; i < hop.nados(); ++i)
        {
            temp = Scoup*ados[i];
            ados[i] = temp;
        }
